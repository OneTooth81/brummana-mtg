...
export default function ManageUsers({ session, onBack }) {
  ...
  const [permissions, setPermissions] = useState({}) // { username: { module: level } }
  ...
  async function setPermission(username, module, level) {
    ...
    await supabase.from('user_permissions').upsert({ username, module, access_level: level }, { onConflict: 'username,module' })
  }
  ...
  async function addUser() {
    ...
    // default permissions: edit members, none accounts
    ...
    <div className="min-h-screen bg-stone-50">
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-stone-200 p-6">
          ...
          <p className="text-sm text-stone-500 mb-4">Set each user's access level per module: <strong>None</strong> hides it, <strong>View</strong> is read-only, <strong>Edit</strong> is full access.</p>
              ...
              <div key={u.username} className="rounded-xl border border-stone-200 p-4">
                <div className="flex items-center justify-between flex-wrap gap-2 mb-3">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {u.username !== ADMIN && <button onClick={() => removeUser(u.username)} className="text-xs text-red-500 hover:bg-red-50 px-2 py-1 rounded">Remove</button>}
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  {MODULES.map(mod => (
                    <div key={mod}>
                      <div className="text-xs text-stone-400 font-medium mb-1 capitalize">{mod === 'members' ? 'Member Database' : 'Account Keeping'}</div>
                      <div className="flex gap-1">
                        {LEVELS.map(lvl => (
                          <button key={lvl} onClick={() => setPermission(u.username, mod, lvl)}
                            className={`flex-1 py-1 rounded-lg border text-xs font-medium capitalize transition ${
                              (permissions[u.username]?.[mod] || 'none') === lvl
        ...
        <div className="bg-white rounded-2xl border border-stone-200 p-6">
          ...
          <div className="space-y-2">
            ...
            <input value={newPin} onChange={e => setNewPin(e.target.value.replace(/\D/g,'').slice(0,8))} placeholder="PIN (optional, 4–8 digits)" type="password" inputMode="numeric"
              className="w-full px-3 py-2 rounded-lg border border-stone-300 focus:outline-none focus:ring-2 focus:ring-teal-500" />
            {error && <p className="text-sm text-red-600 flex items-center gap-1.5"><AlertCircle size={14} /> {error}</p>}
            <button onClick={addUser} disabled={saving} className="w-full py-2.5 rounded-lg bg-teal-700 text-white font-medium hover:bg-teal-800 flex items-center justify-center gap-1.5 disabled:opacity-60">
              <Plus size={17} /> Add user
            </button>
          </div>
        </div>

        <button onClick={onBack} className="w-full py-2.5 rounded-lg border border-teal-700 text-teal-700 font-medium hover:bg-teal-50">← Back to home</button>
      </div>
    </div>
  )
}
